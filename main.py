...
def createImage(count, b):
    ...
    for x in range(0, count):
        ...
        filelist = [ f for f in os.listdir(os.path.join('static', 'bird_images')) if f.endswith(".bak") ]
        for f in filelist:
            os.remove(os.path.join('static', 'bird_images'), f)

        new_image = Image.fromarray(array)
        new_image = new_image.resize(dimensions, resample=0)
        imgname = dirname + '/static/bird_images/' + (str(x)) + '.png'
